<template>
  <div>
    <dev v-if="descriptionFlag">Please add user by clicking following plus button.</dev>
    <br>
    ...
    <v-list light two-line>
      <template v-for="(user,index) in users">
        <v-list-tile :key="user">
          <v-list-tile-action>
            <v-checkbox v-model="selected" :value="user"/>
          </v-list-tile-action>

          <v-list-tile-content>
            <v-text-field v-model="users[index]" :value="user"/>
          </v-list-tile-content>
        </v-list-tile>
        <v-divider v-if="index + 1 < users.length" :key="`user-divider-${index}`"/>
      </template>
    </v-list>

    <v-btn :to="resultUtl">check result</v-btn>
  </div>
</template>
